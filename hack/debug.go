...
// diffs between object versions, interleaved with filtered pod logs from
// controller, agent, scheduler-extender, and kube-scheduler.
//
// Usage:
//
...
//
// Replicated-scheduling profile: when any target is pvc, pv, pod, rv, rsc, rsp,
// lvg, llv, or sc, the tool auto-expands to watch related resources (mc/sds-replicated-volume,
// node, etc.) and streams logs from scheduler-extender (d8-sds-node-configurator) and
// kube-scheduler (kube-system). A one-time snapshot of extender /status, /cache, /stat
// is fetched for slow-scheduling diagnosis.
//
// Flags:
//
...
//	go run hack/debug.go pvc    # triggers replicated-scheduling profile
...
// ---------------------------------------------------------------------------
// Target parsing and profile expansion
// ---------------------------------------------------------------------------
...
// replicatedSchedulingKinds is the set of kinds that trigger the replicated
// scheduling profile expansion (add related resources for slow-scheduling diagnosis).
var replicatedSchedulingTriggerKinds = map[string]bool{
	"pod": true, "pods": true,
	"pvc": true, "pv": true,
	"rv": true, "rsc": true, "rsp": true,
	"lvg": true, "llv": true,
	"sc": true, "storageclass": true,
}

// expandedReplicatedSchedulingKinds is the full set of kinds to watch when
// the replicated scheduling profile is active.
var expandedReplicatedSchedulingKinds = []string{
	"pod", "pvc", "pv", "sc", "rsc", "rsp", "lvg", "llv", "node",
}

// expandTargetsForReplicatedScheduling adds related kinds when any target
// matches the replicated scheduling profile. Preserves explicit targets and
// deduplicates.
func expandTargetsForReplicatedScheduling(targets []target) []target {
	triggered := false
	for _, t := range targets {
		if replicatedSchedulingTriggerKinds[t.kind] {
			triggered = true
			break
		}
	}
	...
	for _, kind := range expandedReplicatedSchedulingKinds {
		if seen[kind] {
			continue
		}
		expanded = append(expanded, target{kind: kind})
		seen[kind] = true
	}

	// Add ModuleConfig/sds-replicated-volume for newControlPlane visibility.
	mcKey := "mc/sds-replicated-volume"
	if !seen[mcKey] {
		expanded = append(expanded, target{kind: "mc", name: "sds-replicated-volume"})
	}
...
// logSource defines a configurable log stream: namespace, label selector, and
// optional raw mode for non-JSON logs (e.g. kube-scheduler).
type logSource struct {
	component     string // display label, e.g. "controller", "scheduler-extender"
	namespace     string
	labelSelector string
	rawMode       bool // if true, pass through lines without JSON parsing
}

// defaultLogSources returns the list of log sources to stream. When
// replicatedScheduling is true, adds scheduler-extender and kube-scheduler.
func defaultLogSources(replicatedScheduling bool) []logSource {
	var sources []logSource
	if replicatedScheduling {
		sources = append(sources,
			logSource{component: "scheduler-extender", namespace: "d8-sds-node-configurator", labelSelector: "app=sds-common-scheduler-extender", rawMode: false},
			logSource{component: "kube-scheduler", namespace: "kube-system", labelSelector: "component=kube-scheduler", rawMode: true},
		)
	}
	return sources
}
...
func followPodLogs(ctx context.Context, src logSource, ws *watchSet, wg *sync.WaitGroup) {
	defer wg.Done()
	...
	watcherWg.Add(1)
	go watchPods(ctx, src.component, src.namespace, src.labelSelector, events, &watcherWg)
	...
	for {
		...
		case ev, ok := <-events:
			...
			case "ADDED", "MODIFIED":
				...
				notifiedWaiting = false
				streamerWg.Add(1)
				go streamPodLogs(podCtx, src, ev.podName, ws, tracker, &streamerWg)
			...
			case "DELETED":
				...
				if len(activePods) == 0 && !notifiedWaiting {
					emit(fmt.Sprintf("%s %s[%s]%s %s── no pods, waiting ──%s",
						ts(), badgeColor(src.component), src.component, colorReset, colorDim, colorReset))
...
func watchPods(ctx context.Context, component, namespace, labelSelector string, events chan<- podEvent, wg *sync.WaitGroup) {
	defer wg.Done()
	defer close(events)
...
func streamPodLogs(ctx context.Context, src logSource, podName string, ws *watchSet, tracker *reconcileIDTracker, wg *sync.WaitGroup) {
	defer wg.Done()
	...
	for {
		...
		for scanner.Scan() {
			...
			if src.rawMode {
				// Pass through non-JSON logs (e.g. kube-scheduler) without filtering.
				if len(jsonStr) > 0 {
					emit(fmt.Sprintf("%s %s[%s]%s %s", ts(), badgeColor(src.component), src.component, colorReset, jsonStr))
				}
				continue
			}
			...
			formatted := formatLogEntryTracked(entry, src.component, tracker)
			emitBlock(formatted)
		}
...
// ---------------------------------------------------------------------------
// Scheduler-extender diagnostic snapshot
// ---------------------------------------------------------------------------

// fetchExtenderSnapshots runs a one-time port-forward to the scheduler-extender
// and fetches /status, /cache, /stat for slow-scheduling diagnosis.
func fetchExtenderSnapshots(ctx context.Context) {
	const ns = "d8-sds-node-configurator"
	const target = "deploy/sds-common-scheduler-extender"
	const remotePort = 8099
	const localPort = 8099

	tag := fmt.Sprintf("%s[extender-snapshot]%s", colorDim, colorReset)

	// Start port-forward in background.
	args := []string{"port-forward", "-n", ns, target, fmt.Sprintf("%d:%d", localPort, remotePort)}
	cmd := exec.CommandContext(ctx, "kubectl", args...)

	// Capture stdout — kubectl writes "Forwarding from ..." when ready.
	stdoutPipe, err := cmd.StdoutPipe()
	if err != nil {
		emit(fmt.Sprintf("%s %s port-forward stdout pipe failed: %v", ts(), tag, err))
		return
	}
	// Capture stderr to surface errors (e.g. service not found).
	var stderrBuf strings.Builder
	cmd.Stderr = &stderrBuf

	if err := cmd.Start(); err != nil {
		emit(fmt.Sprintf("%s %s port-forward start failed: %v", ts(), tag, err))
		return
	}
	defer func() { _ = cmd.Process.Kill() }()

	// Wait for kubectl to signal readiness or timeout.
	...
	case <-time.After(10 * time.Second):
		...
		emit(fmt.Sprintf("%s %s port-forward timed out waiting for readiness. stderr: %s", ts(), tag, stderr))
		return
	case <-ctx.Done():
		return
	}

	// Try HTTPS first (extender may serve TLS), fall back to plain HTTP.
	...
	// Probe /status to detect the scheme.
	addr := fmt.Sprintf("127.0.0.1:%d", localPort)
	scheme := "https"
	cl := tlsClient
	if resp, err := tlsClient.Get(fmt.Sprintf("https://%s/status", addr)); err != nil {
		// HTTPS failed, try plain HTTP.
		if resp2, err2 := plainClient.Get(fmt.Sprintf("http://%s/status", addr)); err2 != nil {
			emit(fmt.Sprintf("%s %s probe failed — https: %v; http: %v", ts(), tag, err, err2))
			return
		} else {
			_ = resp2.Body.Close()
			scheme = "http"
	...
	emit(fmt.Sprintf("%s %s using %s", ts(), tag, scheme))

	base := fmt.Sprintf("%s://%s", scheme, addr)
	for _, path := range []string{"/status", "/cache", "/stat"} {
		url := base + path
		resp, err := cl.Get(url)
		if err != nil {
			emit(fmt.Sprintf("%s %s GET %s: %v", ts(), tag, path, err))
			continue
		}
		var body strings.Builder
		_, _ = io.Copy(&body, resp.Body)
		_ = resp.Body.Close()
		emit(fmt.Sprintf("%s %s%s[extender-snapshot]%s %s:", ts(), colorCyan, "", colorReset, path))
		for _, line := range strings.Split(strings.TrimSpace(body.String()), "\n") {
			if line != "" {
				emit(fmt.Sprintf("  %s%s%s", colorDim, line, colorReset))
			}
		}
	}
}
...
func usage() {
	...
	fmt.Fprintf(os.Stderr, "\nreplicated-scheduling profile (auto-expands when pvc, pv, pod, rv, rsc, rsp, lvg, llv, sc):\n")
	fmt.Fprintf(os.Stderr, "  Adds: scheduler-extender logs (d8-sds-node-configurator), kube-scheduler logs (kube-system),\n")
	fmt.Fprintf(os.Stderr, "  resources: mc/sds-replicated-volume, rsc, rsp, lvg, llv, node; extender /cache, /stat snapshot.\n")
	fmt.Fprintf(os.Stderr, "\nflags:\n")
	...
	fmt.Fprintf(os.Stderr, "  go run hack/debug.go pvc    # triggers replicated-scheduling profile\n")
...
func main() {
	...
	targets = expandTargetsForReplicatedScheduling(targets)
	ws := buildWatchSet(targets)

	replicatedScheduling := false
	for _, t := range targets {
		if replicatedSchedulingTriggerKinds[t.kind] {
			replicatedScheduling = true
			break
		}
	}
	...
	if replicatedScheduling {
		emit(fmt.Sprintf("%s %s[replicated-scheduling]%s profile active: logs from scheduler-extender, kube-scheduler; resources: mc/sds-replicated-volume, rsc, rsp, lvg, llv, node%s",
			ts(), colorCyan, colorReset, colorReset))
		emit(fmt.Sprintf("%s %s[replicated-scheduling]%s slow-scheduling hints: missing %s, zone mismatch, missing RSP/RSC, LVG not ready, insufficient space, stale reservations%s",
			ts(), colorDim, colorReset, "storage.deckhouse.io/sds-replicated-volume-node", colorReset))
	}
	...
	// Start log streamers from configurable sources.
	logSources := defaultLogSources(replicatedScheduling)
	for _, src := range logSources {
		wg.Add(1)
		go followPodLogs(ctx, src, &ws, &wg)
	}

	// One-time extender snapshot for replicated scheduling diagnosis.
	if replicatedScheduling {
		go func() {
			fetchExtenderSnapshots(ctx)
		}()
	}
