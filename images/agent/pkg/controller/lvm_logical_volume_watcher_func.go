...
func deleteLVIfExists(log logger.Logger, sdsCache *cache.Cache, vgName string, llv *v1alpha1.LVMLogicalVolume) error {
	...
	// this case prevents LV with the same name deletion when we are deleting not valid LVMLogicalVolume resource
	if llv.Spec.Thin != nil {
		if lv.PoolName != llv.Spec.Thin.PoolName {
			log.Warning(fmt.Sprintf("[deleteLVIfExists] no need to delete LV %s as it doesnt belong to LVMLogicalVolume %s thin-pool %s", lv.LVName, llv.Name, llv.Spec.Thin.PoolName))
			return nil
		}
	}
