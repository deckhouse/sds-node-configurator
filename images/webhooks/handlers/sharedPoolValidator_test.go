...
func TestWeakerCleanupThanThePoolIsRefused(t *testing.T) {
	v := NewSharedPoolValidator(testClient(t, group(cn.VolumeCleanupRandomFillThreePass, "node-1")))
...
func TestStrongerCleanupThanThePoolIsAllowed(t *testing.T) {
	v := NewSharedPoolValidator(testClient(t, group(cn.VolumeCleanupDiscard, "node-1")))
...
func TestVolumeWithoutAPoolIsRefused(t *testing.T) {
	v := NewSharedPoolValidator(testClient(t))
