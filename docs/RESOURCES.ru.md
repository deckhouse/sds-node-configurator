...
#### Создание LVMVolumeGroup на файловых устройствах

Если выделенных блочных устройств нет, часть существующей файловой системы можно отдать под LVM с помощью файловых устройств (`spec.fileDevices`). Агент создаст файл с предварительно выделенным местом, подключит его как loop-устройство и использует как LVM Physical Volume.

Каждый `spec.fileDevices[].directory` должен находиться внутри базового каталога, заданного параметром модуля `fileDevicesDirectory` (по умолчанию `/opt/deckhouse/sds/file-devices`); пути вне него агент отклоняет, чтобы ошибочная запись не заполнила произвольный путь на узле. Чтобы разместить файлы-подложки на выделенном диске, смонтируйте диск на узле и укажите его точку монтирования в `fileDevicesDirectory` в конфигурации модуля.

Создание группы томов на файле размером 50Gi поверх существующей файловой системы:
...
Совмещение блочных и файловых устройств в одной группе томов:
...
{{< alert level="warning" >}}
Файловые устройства дают накладные расходы из-за двойной косвенности (LVM поверх loop-устройства поверх файловой системы). Используйте их только когда выделенных блочных устройств нет.
{{< /alert >}}

##### Увеличение группы томов на файловых устройствах

Обычный способ — увеличить размер существующей записи:
...
Агент увеличивает файл-подложку, заставляет loop-устройство перечитать его размер и расширяет физический том (PV). Все три шага выполняются online: ничего не размонтируется, ни один логический том не деактивируется, тома под нагрузкой продолжают работать. Thin pool, заданный в процентах от группы томов, расширяется следом автоматически.

Поле `size` можно только увеличивать. Уменьшение отклоняется на уровне admission: вернуть ёмкость — значит уменьшить Volume Group (`pvmove` + `vgreduce`), а это может оказаться невозможным, если на остальных Physical Volume нет места, и деструктивным, если возможно. Для блочных устройств модуль Volume Group тоже не уменьшает.

Если дополнительное место нужно взять с другой файловой системы, добавьте новую запись:

```yaml
  fileDevices:
    - name: data-0        # не меняется
      directory: /opt/deckhouse/sds/file-devices
      size: 50Gi
    - name: data-1        # новый файл, новое loop-устройство, новый PV
      directory: /mnt/second-disk/sds-file-devices
      size: 50Gi
```

Записи различаются именем, поэтому несколько записей одинакового размера могут лежать в одном каталоге.

Запись, которая ещё не была создана на узле (её нет в `status.nodes[].fileDevices` — например, агент её отклонил), можно удалить из спецификации. Удаление записи, на которой уже держится физический том (PV), не поддерживается: модуль никогда не уменьшает группу томов. Агент сообщает о таком удалении в условии `VGConfigurationApplied` и оставляет PV на месте; верните запись обратно либо удалите PV вручную через `pvmove` + `vgreduce` + `pvremove`.

{{< alert level="warning" >}}
Предпочитайте увеличение существующей записи добавлению новых: записей может быть не более 32, и каждая стоит одного loop-устройства, одного PV и метаданных группы томов. Кроме того, толстым (thick) логическим томам, которым нужно непрерывное пространство, тем труднее разместиться, чем сильнее свободное место размазано по множеству мелких PV.
{{< /alert >}}

{{< alert level="info" >}}
Агент автоматически восстанавливает привязку loop-устройств после перезагрузки узла.

Чтобы вернуть узлу место, занятое удалёнными данными, не меняя никаких размеров, используйте `fstrim` — см. [FAQ](./faq.html#как-работают-файловые-устройства-filedevices).
{{< /alert >}}
