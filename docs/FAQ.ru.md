...
## Узел разделяемого пула держит отображение тома, который никому не нужен

Ровно это считает метрика `sds_node_configurator_shared_pool_unlocked_mappings`, и здоровое значение у неё одно — ноль: отображение на узле, не держащем блокировки, — это путь, по которому запись ещё может дойти до тома, принадлежащего другому узлу. Такие отображения узел снимает сам раз в минуту.

Если значение держится выше нуля, значит устройство кем-то открыто: покажет `dmsetup info -c -o name,open | grep '^<vg>-'`, а держателя назовёт `lsof`. Отображение, которым кто-то пользуется, не снимается, и этот отказ сделан намеренно.

## Почему `lvmlockctl --info` на узле пула ничего не печатает?

Потому что он запущен в хостовом mount-namespace, где сокет демона не виден. Команда завершается успешно и выдаёт пустой ответ, который читается как «блокировок нигде нет» и таковым не является. Спрашивать надо демона в его же namespace:
