...
##### Growing a file-backed Volume Group

The straightforward way is to raise the size of an existing entry:
...
The agent grows the backing file, tells the loop device to re-read it and resizes the Physical Volume. All three steps run online: nothing is unmounted, no logical volume is deactivated, and volumes in use keep working throughout. A thin pool sized as a percentage of the Volume Group follows automatically.

`size` can only be increased. Lowering it is rejected at admission: returning capacity means shrinking the Volume Group (`pvmove` + `vgreduce`), which can be impossible when the remaining Physical Volumes have no room and is destructive when it is not. The module does not shrink a Volume Group for block devices either.

When the extra capacity has to come from a different filesystem, add a new entry instead:

```yaml
  fileDevices:
    - name: data-0        # unchanged
      directory: /opt/deckhouse/sds/file-devices
      size: 50Gi
    - name: data-1        # new file, new loop device, new PV
      directory: /mnt/second-disk/sds-file-devices
      size: 50Gi
```

Entries are distinguished by name, so several entries of the same size may share a directory.

An entry that has not been provisioned — it is absent from `status.nodes[].fileDevices`, for example because the agent rejected it — can be removed from the spec. Removing an entry that backs a live Physical Volume is not supported: the module never shrinks a Volume Group. The agent reports such a removal on the `VGConfigurationApplied` condition and leaves the Physical Volume in place; restore the entry, or remove the Physical Volume manually with `pvmove` + `vgreduce` + `pvremove`.

{{< alert level="warning" >}}
Prefer raising an existing entry over adding entries: at most 32 are allowed, and every one of them costs a loop device, a Physical Volume and Volume Group metadata. Thick logical volumes requesting contiguous space also become harder to place as free space is spread across many small Physical Volumes.
{{< /alert >}}

{{< alert level="info" >}}
The agent automatically recovers loop device mappings after a node reboot.

To reclaim space taken by deleted data without changing any size, use `fstrim` — see the [FAQ](./faq.html#how-do-file-backed-devices-filedevices-work).
{{< /alert >}}
