...
Для предотвращения утечек через блочные устройства предусмотрен параметр `volumeCleanup`.
...
* `Discard` (для thick-томов) — все блоки тома будут отмечены как свободные с использованием системного вызова `blkdiscard` перед удалением.
...
> Если выбран метод `Discard`, а устройство не поддерживает `blkdiscard` — том не будет удален. Для его удаления Измените `volumeCleanup` на поддерживаемый, либо очистите диск вручную и удалите настройку `volumeCleanup`.

### Thin-тома

> Очищаются только блоки, занятие thin-томом в момент удаления тома! Если клиентское приложение выполнит `blkdiscard`, например через `fstrim` — данные таких блоков не будут очищены.

В момент освобождения блока thin-тома через `blkdiscard` гостевой операционной системы, эта команда пересылается на устройство. В случае использования жесткого диска или отсутствии поддержки `blkdiscard` со стороны блочного устройства thin-pool, данные останутся в thin-pool до нового использования такого блока. Однако, пользователям предоставляется доступ только к thin-томам, а не к самому thin-пулу. Они могут получить только том из пула, а для thin-томов производится зануление блока thin-pool при новом использовании, что предотвращает утечки между клиентами. Это гарантируется настройкой `thin_pool_zero=1` в LVM.
