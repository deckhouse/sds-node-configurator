...
#### Создание partition на блочном устройстве

Если на блочном устройстве создан partition, контроллер автоматически создаст ресурс `BlockDevice` для этого partition.

Пример создания partition на блочном устройстве:
...
Ресурс `LVMVolumeGroup` может быть создан 3 способами:
...
* Ручное создание VG на узле:
  * Пользователь вручную создает `LVM Volume Group` на узле с помощью стандартных LVM-команд.
  * После создания VG пользователь добавляет LVM-тег `storage.deckhouse.io/enabled=true` для передачи управления контроллеру.
  * Контроллер автоматически обнаружит VG с этим тегом и создаст соответствующий ресурс `LVMVolumeGroup`.
    * Подробная инструкция по добавлению существующей VG в Kubernetes описана в разделе [FAQ](../FAQ.ru.md).
