...
{{< alert level="info" >}}
Обратите внимание, что для полноценной работы модуля требуется включить один из (или оба) модулей [sds-replicated-volume](/modules/sds-replicated-volume/) и [sds-local-volume](/modules/sds-local-volume/).
{{< /alert >}}
